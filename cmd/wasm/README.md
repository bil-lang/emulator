...
cp nodeprog/placedcontroller/roles/deploy.json cmd/wasm/static/roles/deploy.json
```
...
- **No in-page demo selector.** Whichever binary currently sits at `static/node.wasm` (or `static/roles/*.wasm` + `deploy.json`) is what runs — switching demos means rebuilding and refreshing, not picking from a menu.
